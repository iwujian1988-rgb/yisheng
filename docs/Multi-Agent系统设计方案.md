...
特征表：
```
interface AnonymizedDischargeRecordSchema {
  // 1. 诊断信息
  diagnoses: {
    admission_impression: string[]; // 入院诊断 (保留问号)
    discharge_diagnosis: string[];  // 出院诊断 (拆分为数组)
  };

  // 2. 入院情况
  admission_status: {
    reason_of_admission: string;  // 入院原因 (仅提取症状和时间，自动剥离"患者，女，68岁"等身份前缀)
    status_on_admission: string;  // 入院查体情况
  };

  // 3. 住院诊治经过
  hospital_course: {
    lab_tests: Array<{ date: string; test_category: string; results: string }>;
    imaging_and_special_exams: Array<{ date: string; exam_name: string; findings: string }>;
    consultations: Array<{ department: string; diagnosis?: string; suggestions: string }>;
    treatment_summary: string;
  };

  // 4. 出院情况与状态
  discharge_status: {
    status_on_discharge: string;    // 出院时情况
    condition_on_discharge: string; // 出院状态
    disposition: string;            // 出院去向
  };
  ...
  follow_up_instructions: {
    self_care: string;            // 生活自理 (仅提取实心●勾选项)
    activity: string;             // 活动 (仅提取实心●勾选项)
    medication_guidance: string[];// 药物指导 (仅提取实心●勾选项，可能多选)
    diet_guidance: string[];      // 饮食指导 (含特殊饮食指导的实心●勾选项)
    implant_guidance: string;     // 植入物相关指导
    other_instructions: string[]; // 其它指导 (按1,2,3...序号拆分为数组)
  };
  ...
  follow_up_schedule: {
    ...
    outpatient_schedule: string;  // 门诊排班信息
    emergency_symptoms: string;   // 紧急就医指征
  };
}
```
...
| 一级模块                     | 二级字段 (Key)                          | 数据类型   | 提取规则与内容描述 (Agent 约束要点)                                        |
| ------------------------ | ----------------------------------- | ------ | ------------------------------------------------------------- |
| **1. 诊断信息**              | admission_impression (入院诊断)         | Array  | 提取入院时的初步诊断，**必须保留问号(?)** 等不确定表述。                              |
|                          | discharge_diagnosis (出院诊断)          | Array  | **强约束**：按疾病拆分为字符串数组，绝不能揉成一段长文本。                               |
| **2. 入院情况**              | reason_of_admission (入院原因)          | String | 仅提取主诉（症状和时间）。**脱敏要求**：需自动剥离“患者，女，68岁”等前置身份信息。                 |
|                          | status_on_admission (入院查体)          | String | 提取包含生命体征及核心查体体征的完整段落。                                         |
| 3. 住院诊治经过<br><br>(核心高密区) | lab_tests (实验室检查)                   | Array  | **强约束**：按检查时间(如2025-03-20)、项目大类(如血生化)及具体指标切分对象，严禁截断或遗漏。       |
|                          | imaging_and_special_exams (影像/特殊检查) | Array  | 提取心电图、CT、彩超及特殊操作（如馒头餐试验），拆分为时间、名称及结论。                         |
|                          | consultations (专科会诊)                | Array  | 按会诊科室切分。提取科室名称、会诊诊断及处理建议。                                     |
|                          | treatment_summary (诊治经过总结)          | String | 提取病情演变、用药调整逻辑等**纯叙述文本**，需自动剔除夹杂其中的化验单流水。                      |
| **4. 出院情况**              | status_on_discharge (出院时情况)         | String | 简短描述出院时的体征与病情状态。                                              |
|                          | condition_on_discharge (出院状态)       | String | 提取标准转归词汇（如：改善、治愈等）。                                           |
|                          | disposition (出院去向)                  | String | 提取去向（如：回家、转院等）。                                               |
| **5. 出院带药**              | discharge_medications (带药明细)        | Array  | **极端强约束**：将长文本精准打碎成结构化字段。提取：`药名`、`规格`、`带药量`、`单次用量`、`用法`、`频次`。 |
| 6. 出院指导<br><br>(复选框识别)   | self_care (生活自理)                    | String | **复选框约束**：仅提取前缀为实心圆点（●）的勾选项。                                  |
|                          | activity (活动)                       | String | 同上，仅提取选中的项。                                                   |
|                          | medication_guidance (药物指导)          | Array  | 可能存在多选，仅提取实心●选项组成数组。                                          |
|                          | diet_guidance (饮食指导)                | Array  | 整合“饮食指导”与“特殊饮食指导”，仅提取实心●选项。                                   |
|                          | implant_guidance (植入物相关指导)          | String | 提取对应文本（如“无”）。                                                 |
|                          | other_instructions (其它指导)           | Array  | **长列表约束**：按1, 2, 3...等序号，将纯文本长列表精准拆分为字符串数组。                   |
| **7. 复诊安排**              | schedule_table (随访表格)               | Array  | 将类表格文本转换为对象数组，提取时间、地点、目的及科室。                                  |
|                          | outpatient_schedule (门诊排班信息)        | String | 提取门诊具体排班指引。                                                   |
|                          | emergency_symptoms (紧急就医指征)         | String | 提取“以下情况需要紧急就医”下方的症状描述。                                        |

样例：
```
<source_text>
    入院诊断 Impression: 糖尿病2型? 1型? 特殊类型?
    ...
    入院情况 Status on Admission: 查体：呼吸:18次/分钟,脉搏:73次/分钟,血压:112/78mmHg。身高:143cm,体重:40.8Kg,体重指数:20.0。神清，精神可，全身浅表淋巴结未触及肿大，胸廓无畸形...（省略部分查体以节约篇幅）
    ...
    食物/药物间相互作用指导： ▲降糖药：所有降糖药的疗效会受饮食、情绪紧张、运动或其他药物的影响。用药后必须按时进食。平衡膳食及按时进食非常重要。不能与酒精类食品同服
    ...
    4. 腹部B超提示1.肝内脂质沉积；2.胆囊壁毛糙。定期复查腹部B超，消化内科随诊。
    5. 患者肿瘤标记物癌胚抗原CEA 升高，CEA 5.50ng/mL，一月后复查CEA，必要时肿瘤内科随诊。
    6. 患者双眼晶体混浊，眼底可见散在出血点及渗出灶。眼科建议积极控制血糖，羟苯磺酸钙胶囊 1片/次，3次/天。眼科门诊定期复查。不适随诊。
    7. 患者腰椎骨量减少，注意补充钙和维生素D，增加日晒，我科门诊随访。
    8. 膀胱内残余尿约95ml。出院后复查膀胱残余尿量。
    9. 患者大便OB弱阳性，1-2周后门诊复查。
    10. 不适随诊。
...
针对这份病历的 Agent 调优建议（Edge Cases 防护）：

1. **“实验室检查”的数据海啸防线：** 图片中 `尿常规(2025-03-20):葡萄糖:2+;血生化(2025-03-20):钾:3.70...` 这一段极度密集。如果你只用一个 `String` 兜住，大模型可能会因为 Token 生成限制而**擅自总结或截断**后面的肿瘤标志物等数据。
    
    - **Prompt 对策**：必须在 Prompt 中强调“`lab_tests` 必须全量遍历原文本，**严禁省略、概括或截断**任何化验指标”。
        
2. **“出院带药”的正则化提取：** 带药文本是没有换行的流式文本：“`[来得时]甘精胰岛素针 300iu:3mlX1 带药量0支 每次14iu 皮下注射 每晚一次 [集采]瑞格列奈片...`”
    
    - **Prompt 对策**：在 Few-Shot 样例中，务必给出一个将这种“连体婴”文本精准切分为 JSON Array 的正向示例，让模型学会通过 `[xxx]` 和 `每次/每晚` 等关键字作为分隔符来切分不同药品。
...
特征表、
```
interface SeventyTwoHourTalkRecordSchema {
  // 1. 简要病情 (融合了主诉、现病史、既往史及查体)
  brief_condition: {
    history_and_symptoms: string; // 病史及症状描述 (提取第1、2、3点，自动剥离"1.患者，女，68岁"等前缀)
    physical_exam: string;        // 查体情况 (提取第4点内容)
    auxiliary_exams_on_admission: string; // 入院辅助检查 (提取第5点内容，如随机血糖)
  };

  // 2. 入院后的检查结果 (高密度数据区)
  // 注意：原文本中实验室检查与影像学检查混写在一段，需 Agent 强行拆解
  test_results_since_admission: {
    lab_tests: Array<{
      date: string;          // 检查日期 (如：2025-03-20)
      test_category: string; // 检验大类 (如：尿常规, 血生化, 凝血功能)
      results: string;       // 具体指标及数值
    }>;
    imaging_and_special_exams: Array<{
      date: string;          // 检查日期
      exam_name: string;     // 检查名称 (如：心电图, 腹部CT, 各种彩超)
      findings: string;      // 检查结果及结论
    }>;
  };

  // 3. 综合诊断
  current_diagnosis: string[]; // 目前考虑疾病诊断 (将文本拆分为数组，如：["2型糖尿病", "糖尿病大血管病变"...])

  // 4. 住院期间可能出现的风险和疾病常见并发症 (长列表提取)
  potential_risks_and_complications: string[]; // 按 1, 2, 3... 序号提取为字符串数组

  // 5. 诊疗方案
  treatment_plan: {
    nursing_and_diet: string;    // 护理与饮食 (如：内科护理常规，二级护理，糖尿病饮食)
    further_exams: string;       // 进一步检查计划 (如：完善24小时尿蛋白定量...)
    treatment_strategy: string;  // 治疗策略 (如：治疗上予以胰岛素皮下泵控制血糖...)
    discharge_planning: string;  // 可衡量的目标和出院计划 (如：住院期间进行饮食指导...)
  };
}
```
...
|**一级模块**|**二级字段 (Key)**|**数据类型**|**提取规则与内容描述 (Agent 约束要点)**|
|---|---|---|---|
|**1. 简要病情**|history_and_symptoms|String|提取文本中编号 1-3 的内容。**强制脱敏要求**：自动识别并删除类似“患者，女，68岁”的身份前缀信息。|
||physical_exam|String|仅提取“4.查体：”后面的生命体征和体格检查段落。|
||auxiliary_exams_on_admission|String|提取“5.辅助检查：”后面的内容。|
|2. 检查结果<br><br>  <br><br>(混合重组区)|lab_tests|Array|**强约束**：原文本是一整段长文本，Agent 必须根据括号内的时间（如 2025-03-20）和冒号前的大类词（如血生化、尿常规），将其**拆解重组**为结构化数组对象。|
||imaging_and_special_exams|Array|同上，从长段落的后半部分提取心电图、BMD、彩超、CT等影像和操作检查，重组为数组对象。|
|**3. 诊断**|current_diagnosis|Array|提取“目前考虑疾病诊断为：”下方的疾病列表，按空格或换行拆分为独立的字符串数组。|
|**4. 风险告知**|potential_risks_and_complications|Array|将“住院期间可能出现风险和疾病常见并发症如下：”下方的长段落，严格按照“1.”、“2.”等序号，切分为多个独立的风险条款数组。|
|**5. 诊疗方案**|nursing_and_diet|String|提取“诊疗方案：”下第1点内容。|
||further_exams|String|提取第2点内容。|
||treatment_strategy|String|提取第3点内容。|
||discharge_planning|String|提取第4点内容。|
...
<example>
  ...
  <expected_output>
    {
      "brief_condition": {
        "history_and_symptoms": "因“口干多饮多尿6年余，加重1周”入院。患者6年余前体检时发现空腹血糖10+mmol/L,伴口干，多饮，饮水量1-2L每天，多尿，夜尿1-2次每天，当地医院予“口服药”（具体不详）治疗，监测血糖空腹6-7mmol/L，一周后停用降糖药。6年来未服药，未监测血糖。1周前患者夜尿次数较前增加，3-4次每天，口干多饮症状同前，双脚大足趾发麻，夜间明显，外院(2025.03.19)查空腹血糖15.66mmol/L,糖化13.3%，总胆固醇6.65mmol/L,低密度脂蛋白4.29mmol/L,尿糖4+。既往：无殊。",
        "physical_exam": "血压:176/90mmHg,脉搏:84次/分钟,体温（耳）:36.2℃,呼吸:20次/分钟,身高:143cm,体重:40.8Kg,体重指数:20.0。神清，精神可，全身浅表淋巴结未触及肿大，胸廓无畸形，双肺呼吸音清，未闻及干湿啰音，心界无扩大，心律齐，心前各瓣膜区未闻及病理性杂音。腹软，无压痛反跳痛，肝脾肋下未及。双下肢无浮肿，双侧足背动脉搏动正常可及。双侧肢体温度觉、痛觉、压力觉、位置觉、振动觉未见明显异常。",
        "auxiliary_exams_on_admission": "入院随机血糖:20.3mmol/L"
      },
      "test_results_since_admission": {
        "lab_tests": [
          { "date": "2025-03-20", "test_category": "尿常规", "results": "葡萄糖:2+" },
          { "date": "2025-03-20", "test_category": "血生化", "results": "糖化血清白蛋白:34.0%;钾:3.70mmol/L;肌酐:49μmol/L;尿素/肌酐:0.11;总胆固醇:5.99mmol/L;高密度脂蛋白胆固醇:1.41mmol/L;低密度脂蛋白胆固醇:3.90mmol/L;脂蛋白(a):15.6mg/dL;尿酸:218.9μmol/L;肌酸激酶:133U/L;超敏C反应蛋白:0.4mg/L;eGFR(CKD-EPI):96.4mL/min" },
          { "date": "2025-03-20", "test_category": "糖化血红蛋白", "results": "HbA1c:13.40%" },
          { "date": "2025-03-20", "test_category": "凝血功能", "results": "凝血酶时间(TT):19.5s" },
          { "date": "2025-03-20", "test_category": "肿瘤标志物", "results": "癌胚抗原CEA:5.45ng/mL;铁蛋白Fer:847.90μg/L" },
          { "date": "2025-03-20", "test_category": "术前免疫", "results": "乙肝病毒表面抗体定量:19.39IU/L;乙型肝炎病毒核心抗体:4.71S/CO" },
          { "date": "2025-03-20", "test_category": "血常规,ABO,RH", "results": "ABO:B型;Rh(D):阳性;白细胞计数:5.6x10^9/L;红细胞计数:4.56x10^12/L;血红蛋白量:147g/L;血小板计数:213x10^9/L;中性粒细胞绝对数:3.47x10^9/L;淋巴细胞绝对数:1.46x10^9/L" },
          { "date": "2025-03-20", "test_category": "大便常规", "results": "隐血试验:弱阳性(+-)" },
          { "date": "未知", "test_category": "其它异常项筛查", "results": "微量尿白蛋白/尿肌酐比值、TGAb,TPOAb,甲状腺功能、ICA,IAA,GAD,IA-2、24小时尿蛋白、微量白蛋白未见明显异常。" }
        ],
        "imaging_and_special_exams": [
          { "date": "2025-03-19", "exam_name": "心电图", "findings": "窦性心律（83次/分）;高侧壁、前侧壁ST-T改变" },
          { "date": "2025-03-20", "exam_name": "BMD", "findings": "腰椎骨量减少;详见图文报告;感觉阈值：L7.8V/R7.4V" },
          { "date": "2025-03-20", "exam_name": "肝|胆|胰|脾彩超", "findings": "1.肝内脂质沉积2.胆囊壁毛糙" },
          { "date": "2025-03-20", "exam_name": "胸部CT平扫", "findings": "右肺多发小结节，必要时复查。左侧斜裂结节样增厚。左侧胸膜结节状突起。两肺少许纤维灶" },
          { "date": "2025-03-20", "exam_name": "女泌尿系统(肾|输尿管|膀胱)彩超", "findings": "双肾、输尿管、膀胱超声未见明显异常" },
          { "date": "2025-03-20", "exam_name": "双侧颈动脉彩超", "findings": "双侧颈动脉内膜毛糙增厚伴双侧斑块形成" },
          { "date": "2025-03-20", "exam_name": "右下肢动脉彩超|左下肢动脉彩超", "findings": "双下肢动脉内膜毛糙" },
          { "date": "2025-03-20", "exam_name": "膀胱残余尿量测定", "findings": "膀胱内残余尿约95ml" },
          { "date": "2025-03-21", "exam_name": "肌电图", "findings": "NCV+EMG提示：未见明显异常" }
        ]
      },
      "current_diagnosis": [
        "2型糖尿病",
      ...
      "potential_risks_and_complications": [
        "患者住院期间可能由于感染、应激、创伤或药物（激素等）等出现明显的血糖波动或血糖难以控制；出现酮症酸中毒或高渗性昏迷不能迅速缓解等；出现低血糖症，可有头晕心慌手抖等表现，或夜间无症状的低血糖，严重时出现昏迷；",
        "糖尿病可并发糖尿病周围神经病变、糖尿病视网膜病变，严重时可能会出现感觉异常、四肢麻木，视物模糊、头晕、直立性低血压等不适，导致摔倒，出现意外伤；并发糖尿病肾病，可出现蛋白尿、浮肿、低蛋白血症、高血压，进一步发展出现肾功能衰竭，需要透析等；出现糖尿病大血管病变，进一步发展可能出现急性冠脉综合症、TIA及脑卒中等心脑血管意外；可出现糖尿病足病，可出现下肢动脉闭塞，严重时可能需要截肢手术；",
        "糖尿病患者易发生各种感染或院内感染，感染不易控制，严重时出现感染性休克；",
        "糖尿病患者多合并多种并发症或合并症，服用多种药物，用药期间出现各种药物不良反应：如肝肾功能损害、胃肠道反应、过敏等；",
        "检查发现其他疾病及其它并发症可能，发现良恶性肿瘤可能，需转外科手术治疗等；",
        "糖尿病患者多合并多种并发症或合并症，病情变化快，容易出现一些不可预料的意外情况或并发症。"
      ],
      "treatment_plan": {
        "nursing_and_diet": "内科护理常规，二级护理，糖尿病饮食",
        "further_exams": "完善24小时尿蛋白定量、肌电图、馒头餐试验等检查，继续并发症评估",
        "treatment_strategy": "治疗上予以胰岛素皮下泵控制血糖。根据血糖及检查结果调整治疗方案。",
        "discharge_planning": "可衡量的目标和出院计划：住院期间进行糖尿病饮食指导，知晓糖尿病相关知识，出院后糖尿病饮食控制，规律用药，定期监测血糖，注意低血糖，知晓低血糖应对方法，内分泌科门诊定期随访。"
      }
    }
  </expected_output>
</example>
...
特征表：
```
interface AnonymizedAdmissionRecordSchema {
  // 1. 核心病史
  medical_history: {
    chief_concern: string;            // 主诉
    history_of_present_illness: string; // 现病史
  };

  // 2. 过去史与系统回顾 (将零散的否认项聚合并提取关键信息)
  past_history: {
    general_health: string;           // 一般健康状况
    chronic_diseases: string[];       // 内科疾病史 (提取"有"的项，如["糖尿病"])
    surgical_and_trauma: string;      // 手术外伤史
    blood_transfusion: string;        // 输血史
    allergy_history: string;          // 过敏史
    vaccination_history: string;      // 预防接种史
    other_history: string[];          // 其他病史 (传染病、中毒等，有阳性时提取)
  };

  // 3. 个人史、婚育史与家族史
  personal_and_family_history: {
    birth_and_residence: string;      // 出生、生长史及原籍
    habits: {
      smoking: string;                // 吸烟史
      drinking: string;               // 饮酒史
    };
    occupational_exposure: string;    // 工作性质及毒物接触
    marriage_and_childbearing: {
      status: string;                 // 婚姻状况 (如：已婚)
      childbearing_history: string;   // 生育史 (如：1-0-2-1)
      menstrual_history: string;      // 月经史/绝经状态
    };
    family_history: {
      similar_diseases: string;       // 家庭成员类似病史
      genetic_diseases: string;       // 两系三代遗传性疾病 (如：糖尿病)
    };
  };

  // 4. 体格检查 (极高密度嵌套区 - 采用"异常聚焦"提取策略)
  // 约束：对于全是"无明异常/未及"的系统，仅提取概括性描述，重点提取具体数值和阳性体征
  physical_examination: {
    vital_signs: {
      temperature: string;            // 体温 (℃)
      pulse: string;                  // 脉搏 (次/分)
      respiration: string;            // 呼吸 (次/分)
      blood_pressure: string;         // 血压 (mmHg)
      bmi: string;                    // BMI
    };
    general_status: string;           // 一般情况 (如：神志清醒，发育正常，营养良好...)
    lymph_nodes: string;              // 浅表淋巴结
    head_and_neck: string;            // 头部及颈部 (包含五官、颈部血管、甲状腺等)
    chest_and_lungs: string;          // 胸部和肺 (视触叩听结论)
    cardiovascular: {
      heart_rate: string;             // 心率
      rhythm: string;                 // 心律
      murmur: string;                 // 杂音
      heart_border: string;           // 叩诊心界 (如：左锁骨中线距前正中线8cm)
      peripheral_vessels: string;     // 周围血管征
    };
    abdomen: {
      inspection_palpation: string;   // 腹部视诊与触诊 (柔软度、压痛、包块、肝脾触诊等)
      percussion_auscultation: string;// 腹部叩诊与听诊 (移动性浊音、肠鸣音等)
    };
    spine_and_limbs: string;          // 脊柱与四肢
    neurological: string;             // 神经系统 (生理反射、病理征等)
  };

  // 5. 专科检查与辅助检查
  specialized_and_auxiliary_exams: {
    special_exam_of_area: string;     // 专科检查 (提取具体描述段落)
    diagnostic_tests: {
      laboratory: string;             // 实验室检查 (原表为斜杠，若有内容则提取，否则为空)
      imaging: string;                // 影像学检查
      other: string;                  // 其他检查 (如：患者入院血糖20.3mmol/L)
    };
  };

  // 6. 诊断结论
  diagnoses: {
    impression: string[];             // 初步诊断 (按疾病拆分，保留问号)
    revised_diagnosis: string[];      // 修正诊断 (若有)
    consultation_needs: string[];     // 会诊需求 (如：["营养会诊: 需要"])
  };
}
```
...
| 一级模块                  | 二级字段 (Key)                       | 数据类型   | 提取规则与内容描述 (Agent 约束要点)                                                                                |
| --------------------- | -------------------------------- | ------ | ----------------------------------------------------------------------------------------------------- |
| **1. 核心病史**           | chief_concern (主诉)               | String | 提取患者的主要症状及持续时间。                                                                                       |
|                       | history_of_present_illness (现病史) | String | 提取完整段落。**脱敏约束**：需自动剥离段首可能出现的“患者XXX”等具体姓名身份前缀。                                                         |
| **2. 过去史等**           | general_health (一般健康状况)          | String | 提取如“良好”、“一般”等词汇。                                                                                      |
|                       | chronic_diseases (内科疾病史)         | Array  | **强约束 (信息过滤)**：只提取勾选为“有”的疾病（如`["糖尿病"]`）。坚决丢弃标为“否认”的疾病项，不要输出类似`["高血压否认", "心脏病否认"]`的无用信息。               |
|                       | surgical_and_trauma (手术外伤史)      | String | 提取具体手术描述（如“40年余前行阑尾切除术”）。若均为否认，填“否认”。                                                                 |
|                       | blood_transfusion (输血史)          | String | 提取有无输血。                                                                                               |
|                       | allergy_history (过敏史)            | String | 提取食物和药物过敏情况。                                                                                          |
|                       | vaccination_history (预防接种史)      | String | 提取接种描述（如“随社会”）。                                                                                       |
|                       | other_history (其他病史)             | Array  | 针对传染病、中毒史、疫区史等，**仅在有阳性发现时提取归纳入数组**，全阴性则返回空数组 `[]`。                                                    |
| **3. 个人/家族史**         | birth_and_residence (出生及原籍)      | String | 提取出生地、生长史。                                                                                            |
|                       | habits (个体习惯)                    | Object | 分别提取 `smoking`(吸烟) 和 `drinking`(饮酒) 的状态。                                                              |
|                       | occupational_exposure (职业暴露)     | String | 合并“工作性质”与“毒物接触”项。                                                                                     |
|                       | marriage_and_childbearing (婚育月经) | Object | 提取 `status`(婚姻)、`childbearing_history`(如1-0-2-1) 及 `menstrual_history`(初潮/周期/绝经等概括)。                  |
|                       | family_history (家族史)             | Object | 分别提取 `similar_diseases`(家庭成员类似病史) 和 `genetic_diseases`(遗传病史)。                                         |
| 4. 体格检查<br><br>(最高密区) | vital_signs (生命体征)               | Object | 精准提取数值：体温、脉搏、呼吸、血压、BMI。不要带单位。                                                                         |
|                       | general_status (一般情况)            | String | 将神志、体位、发育、营养、面容等独立小项，**拼装成通顺的概括性短句**（如：“神志清醒，发育正常...”）。                                               |
|                       | lymph_nodes (浅表淋巴结)              | String | 概括提取（如：“全身浅表淋巴结未及肿大”）。                                                                                |
|                       | head_and_neck (头颈部)              | String | **合并压缩约束**：将头、眼、耳、鼻、口、颈部的海量小项（全为“无明异常/未及”时）合并为一两句话。若有具体数值（如“瞳孔左右侧均3mm”）必须单独提炼。                        |
|                       | chest_and_lungs (胸部和肺)           | String | 合并胸廓视诊与肺部触叩听结论。                                                                                       |
|                       | cardiovascular (心脏及血管)           | Object | 提取 `heart_rate`(心率数值)、`rhythm`(心律)、`murmur`(杂音)。**必须精确提取** `heart_border`(叩诊心界距离数值，如"左锁骨中线距前正中线8cm")。 |
|                       | abdomen (腹部)                     | Object | 拆分为视触(如腹肌柔软度、肝脾触诊)与叩听(如肠鸣音、移动性浊音)。                                                                    |
|                       | spine_and_limbs (脊柱四肢)           | String | 概括形态、关节活动及病理反射。                                                                                       |
|                       | neurological (神经系统)              | String | 概括生理反射与病理征提取结果。                                                                                       |
| **5. 专科及辅检**          | special_exam_of_area (专科检查)      | String | 提取“专科检查”下的完整描述段落。                                                                                     |
|                       | diagnostic_tests (辅助检查)          | Object | 包含化验、影像和其他。**约束**：若原文本中对应项只有斜杠“/”或空白，该字段应填空字符串 `""`；若有手写/录入文字（如“患者入院血糖20.3...”）则全量提取。                 |
| **6. 诊断结论**           | impression (初步诊断)                | Array  | 提取末尾的诊断结论。**约束**：按疾病名词切分为数组，若带有“?”必须保留以体现待排查状态。                                                       |
|                       | revised_diagnosis (修正诊断)         | Array  | 若为空白则填空数组 `[]`。                                                                                       |
|                       | consultation_needs (会诊需求)        | Array  | 提取康复、营养会诊结论（如：`["营养会诊: 需要"]`）。不要提取“不需要”的项。                                                            |
...
<example>
  <source_text>
    ...
    ... 个体习惯：吸烟：否认 饮酒：已戒酒
    婚姻、月经及生育史 ... 结婚年龄:26岁 生育史:1-0-2-1(足-早-流-存) 月经史:15 [5-6/30] 47 月经量及性质:绝经
    ...
    ... (此处省略长串的皮肤、淋巴结、五官等全阴性查体) ...
    心脏检查：视诊:心前区隆起 无 心尖搏动 可及 ... 叩诊:实测数据见叩诊表 左锁骨中线距前正中线8cm ... 听诊:心率(次/分) 84 心律 齐 ...
    腹部检查：... 腹肌紧张度 柔软 压痛 无 ... 肝界位于右锁骨中线第5肋间 ... 肠鸣音 正常 ...
    专科检查
    查体：血压:176/90mmHg,脉搏:84次/分钟,体温（耳）:36.2℃,呼吸:20次/分钟,身高:143cm,体重:40.8Kg,体重指数:20.0。神清，精神可...双侧肢体温度觉、痛觉、压力觉、位置觉、振动觉未见明显异常。
    辅助检查
  ...
  <expected_output>
    {
      "medical_history": {
        "chief_concern": "口干多饮多尿6年余，加重1周",
        "history_of_present_illness": "6年余前体检时发现空腹血糖10+mmol/L,伴口干，多饮，饮水量1-2L每天，多尿，夜尿1-2次每天，无泡沫尿，无恶心呕吐，心慌发抖等症状，无视物模糊，当地医院予“口服药”（具体不详）治疗，监测血糖空腹6-7mmol/L，一周后停用降糖药。6年来未服药，未监测血糖。1周前夜尿次数较前增加，3-4次每天，口干多饮症状同前，双脚大足趾发麻，夜间明显，外院(2025.03.19)查空腹血糖15.66mmol/L,糖化13.3%，总胆固醇6.65mmol/L,低密度脂蛋白4.29mmol/L,尿糖4+。为进一步诊治来我院，门诊拟“糖尿病”收住入科。自病以来，神清，精神可，胃纳可，睡眠差，小便如上述，大便无殊，体重3年下降5kg。"
      },
      "past_history": {
        "general_health": "良好",
        "chronic_diseases": ["糖尿病"],
        "surgical_and_trauma": "40年余前行阑尾切除术",
        "blood_transfusion": "否认",
        "allergy_history": "未发现",
        "vaccination_history": "随社会",
        "other_history": []
      },
      "personal_and_family_history": {
        "birth_and_residence": "原籍",
        "habits": {
          "smoking": "否认",
          "drinking": "已戒酒"
        },
        "occupational_exposure": "退休人员，毒物接触否认",
        "marriage_and_childbearing": {
          "status": "已婚",
          "childbearing_history": "1-0-2-1(足-早-流-存)",
          "menstrual_history": "初潮15岁，周期30天，经期5-6天，47岁绝经"
        },
        "family_history": {
          "similar_diseases": "三个哥哥和弟弟均糖尿病",
          "genetic_diseases": "糖尿病"
        }
      },
      "physical_examination": {
        "vital_signs": {
          ...
          "pulse": "84",
          "respiration": "20",
          "blood_pressure": "176/90",
          "bmi": "20"
        },
        "general_status": "神志清醒，体位自主，发育正常，营养良好，面容无病容，表情自如。",
        "lymph_nodes": "全身浅表淋巴结未及肿大。",
        "head_and_neck": "头颅大小无明显异常。眼瞳孔左右侧均3mm，对光反射正常。耳鼻咽喉口腔未见明显异常。颈软，甲状腺未及明显异常。",
        "chest_and_lungs": "胸廓正常，双侧呼吸运动对称，两肺叩诊清音，双侧呼吸音清且对称。",
        "cardiovascular": {
          "heart_rate": "84",
          "rhythm": "齐",
          "murmur": "无",
          "heart_border": "左锁骨中线距前正中线8cm",
          "peripheral_vessels": "桡动脉搏动84次/分，节律规则，足背动脉搏动正常"
        },
        "abdomen": {
          "inspection_palpation": "腹部平坦，腹软，无压痛及反跳痛，肝脾肋下未及，未触及包块，Murphy征阴性。",
          "percussion_auscultation": "肝上界位于右锁骨中线第5肋间，移动性浊音阴性，肠鸣音正常。"
        },
        "spine_and_limbs": "脊柱形态未见异常，四肢关节无红肿，活动功能无异常。",
        "neurological": "生理反射存在，病理征未引出，脑膜刺激征阴性。"
      },
      "specialized_and_auxiliary_exams": {
        "special_exam_of_area": "血压:176/90mmHg,脉搏:84次/分钟,体温（耳）:36.2℃,呼吸:20次/分钟,身高:143cm,体重:40.8Kg,体重指数:20.0。神清，精神可，全身浅表淋巴结未触及肿大，胸廓无畸形，双肺呼吸音清，未闻及干湿啰音，心界无扩大，心律齐，心前各瓣膜区未闻及病理性杂音。腹软，无压痛反跳痛，肝脾肋下未及。双下肢无浮肿，双侧足背动脉搏动正常可及。双侧肢体温度觉、痛觉、压力觉、位置觉、振动觉未见明显异常。",
        "diagnostic_tests": {
          "laboratory": "",
          "imaging": "",
          "other": "患者入院血糖20.3mmol/L。"
        }
      },
      "diagnoses": {
        "impression": ["糖尿病", "2型?", "1型?", "特殊类型?"],
        "revised_diagnosis": [],
        "consultation_needs": ["营养会诊: 需要"]
      }
    }
  </expected_output>
</example>
...
特征表：
```
interface AnonymizedConsultationRecordSchema {
  // 1. 疾病背景信息
  disease_context: {
    syphilis_consultation: string; // 梅毒会诊 (如："否")
    current_diagnosis: string[];   // 当前诊断 (按疾病名词拆分为数组)
  };

  // 2. 会诊意见 (核心内容区)
  consultation_opinion: {
    clinical_evaluation: string;   // 临床评估 (提取病史、饮食现状、体重变化及能量推荐等前置评估内容)
    nrs2002_score?: number;        // NRS2002评分 (若文本包含该评分，提取纯数字；若无则为 null)
    suggestions: string[];         // 具体建议 (按1、2、3等序号拆分为字符串数组)
  };

  // 3. 注意事项与知情同意
  precautions_and_consent: string; // 注意事项 (如："已予以口头营养咨询，表示知晓。")
}
```
...
| 一级模块                   | 二级字段 (Key)                     | 数据类型   | 提取规则与内容描述 (Agent 约束要点)                                                         |
| ---------------------- | ------------------------------ | ------ | ------------------------------------------------------------------------------ |
| **1. 疾病背景**            | syphilis_consultation (梅毒会诊)   | String | 提取“梅毒会诊”对应状态（如：“否”）。                                                           |
|                        | current_diagnosis (当前诊断)       | Array  | 提取原文本下的诊断，按疾病名词切分为字符串数组。                                                       |
| 2. 会诊意见<br><br>(内容重组区) | clinical_evaluation (临床评估)     | String | 提取“会诊意见”栏内，关于患者现状、病史、评估结论的**前置叙述文本**（在“建议如下”之前的段落）。需要自动过滤掉无实际临床意义的客套话（如“谢邀~”）。 |
|                        | nrs2002_score (评分)             | Number | **精准提取**：若“会诊意见”中包含“NRS2002评分X分”，仅提取数字。若无此内容，返回 `null`。                        |
|                        | suggestions (具体建议)             | Array  | **列表拆解约束**：将“建议如下”后方的文本，按“1、”，“2、”等序号强制拆分为独立数组元素。                              |
| **3. 注意事项**            | precautions_and_consent (注意事项) | String | 提取“注意事项”栏内的完整文本，通常为知情告知或宣教记录。                                                  |
...
example>
  ...
  <expected_output>
    {
      "disease_context": {
        "syphilis_consultation": "否",
        "current_diagnosis": [
          "2型糖尿病伴血糖控制不佳"
        ]
      },
      "consultation_opinion": {
        "clinical_evaluation": "患者现食欲胃纳可，饮食摄入满足日常所需80-100%， 早餐以牛奶、鸡蛋和馒头为主，中晚餐以荤素+米饭/面条为主，近2年体重下降约5kg（下降幅度11%），目前推荐能量1200Kcal/d。",
        "nrs2002_score": 2,
        "suggestions": [
          "1、维持糖尿病普食；",
          "2、营养宣教：已详细指导患者及家属饮食合理分配及适宜量，避免易升糖食物及脂肪含量较高食物摄入，控制淀粉类食物总摄入量，解答其饮食疑问等。"
      ...
      "precautions_and_consent": "已予以口头营养咨询，表示知晓。"
    }
  </expected_output>
</example>
```
