...
const config = {
  ...
  ocrStructuredModel: process.env.OCR_STRUCTURED_MODEL || 'qwen3-vl-flash',
  ...
  // OCR is a DashScope-native API. Never prefer a separate OpenAI-compatible
  // text-provider key (for example DeepSeek) when both providers are configured.
  dashscopeApiKey: process.env.DASHSCOPE_API_KEY || process.env.ASR_CLOUD_API_KEY || process.env.AI_API_KEY || '',
  ...
  asrCloudApiKey: process.env.ASR_CLOUD_API_KEY || process.env.DASHSCOPE_API_KEY || process.env.AI_API_KEY || '',
  ...
  aiThinkingMode: process.env.AI_THINKING_MODE === 'enabled' ? 'enabled' : 'disabled',
